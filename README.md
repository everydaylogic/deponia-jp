...
配布データに含まれるバッチファイル等はクレユニさんのツールを元にしたものです。
同梱についてはご本人の快諾をいただいています。

> **翻訳について:** プレイしながら可能な範囲で表示を確認していますが、
> すべてのテキストを確認できたわけではありません。
> 誤訳や未翻訳、表示崩れが残っている可能性があります。
...
`14_modify_vbin.bat` などを実行すると、次のような警告が大量に表示されます。

```
WARNING: パスが違います: フォント9
```

**これらは無視して問題ありません。**Deponia シリーズではフォント定義を変更しているため
出るもので、クレユニさんの README にも無視してよいと明記されています。
...
> **文字数の目安:** 元の文字を全部残したうえで追加できるのは 1500 文字程度が限界とされています。
...
クレユニさんの README には、元からある文字を全部残したうえで追加できるのは
**1500 文字程度が限界** と記載があります。
これを超えると、ゲーム画面に表示されない文字が発生します。
実際、翻訳文で使用している漢字が多く、この状態になりました。

文字数が多すぎると、ゲーム起動時やフォント読み込み時に長い待ち時間が発生することもあります。
...
`14_modify_vbin.bat` の実行中に出る `WARNING: パスが違います: フォント9` のような
警告は、無視して構いません。Deponia シリーズではフォント定義を変更しているために
出るもので、処理は最後まで進み、必要なファイルも作成されます。
